...
import argparse
import torch
from src2.env import create_train_env
from src2.model import ActorCritic, ICM
from src2.process import local_train, local_test
from src2.glob_optim import GlobalAdam as optim
import torch.multiprocessing as _mp
import shutil

def get_args():
    parser = argparse.ArgumentParser("""Implementation of A3C+ICM, proposed by the paper "Curiosity-driven Exploration by Self-supervised Prediction" """)
    parser.add_argument('--lambda_forward', type=float, default=0.2, help='parameter for ICM loss')     ##
    parser.add_argument('--eta', type=float, default=100, help='parameter for scaling intrinsic reward')  ##
    ...
    parser.add_argument("--max_actions", type=int, default=300, help="Maximum repetition steps in test phase")
...
def train(opt):
    ...
    for index in range(opt.num_processes):
        icm = ICM(num_inputs=4, num_actions=num_actions)  #each process has its own icm, not shared
        icm.train() 
        if index == 0:
            process = mp.Process(target=local_train, args=(index, opt, global_model, optimizer, icm, True))  #### added icm
        else:
            process = mp.Process(target=local_train, args=(index, opt, global_model, optimizer, icm))
        process.start()
